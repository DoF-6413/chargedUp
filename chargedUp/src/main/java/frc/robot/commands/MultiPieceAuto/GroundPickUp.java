...
public class GroundPickUp extends SequentialCommandGroup {
  ...
  public GroundPickUp(TelescoperSubsystem telscoper, ArmPIDSubsystem arm, EndEffectorSubsystem NEfector) {
    ...
    addCommands(
        ...
        Commands.runOnce(
            () -> {
              arm.setGoal(Units.degreesToRadians(ArmConstants.kfloorCube) + ArmConstants.kArmOffsetRads); //ArmConstants.kfloorCube
              arm.enable();
        ...
        Commands.runOnce(
            () -> {
              arm.setGoal(Units.degreesToRadians(40) + ArmConstants.kArmOffsetRads); //40
              arm.enable();
            },
            arm),
        new WaitUntilCommand(() -> arm.atGoal()),
